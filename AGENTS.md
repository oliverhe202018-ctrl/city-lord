...
4. **核心资产保护原则**: 在修改时，务必确认是否必须修改核心资产，包括后台定位关屏保活、领地闭合、领地色块显示等业务核心。如果不是必须修改，绝对不允许动这些板块的代码。
5. **移动端优先**: 视觉设计必须是移动端 Native 体验，严禁出现滚动条、右键菜单、不适配的 Hover 态。
6. **性能敏感**: 领地合计数据量大，优先使用服务端聚合或缓存。明确区分“实时数据”与“每日更新数据”。
7. **地理准确性**: 涉及坐标处必须核实坐标系（GCJ-02 与 WGS-84 转换），防止领地偏移。
8. **数据与安全同频**：Prisma Schema 不是孤立的。任何涉及删除、重命名数据表或列的操作，**必须提前预判 Supabase RLS（行级安全策略）的依赖冲突**，禁止直接暴力抛出不可回滚的 Migration。
