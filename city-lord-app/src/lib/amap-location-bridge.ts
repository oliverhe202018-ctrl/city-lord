...
const DEFAULT_ACCURACY_THRESHOLD = 100;
/** 冷启动时允许的精度门槛（更宽松） */
...
export class AMapLocationBridge {
    ...
    /** 定位推送节流间隔 (ms) —— 保护 Android Binder 资源 */
    private static EMIT_THROTTLE_MS = 900;
    ...
    async init(): Promise<void> {
        ...
        if (this.isNative) {
            ...
            try {
                // 使用 ESM 动态导入确保与 Capacitor 代理机制兼容
                const { AMapLocation } = await import('@/plugins/amap-location/definitions');
                if (!AMapLocation) {
                    throw new Error('AMapLocation plugin not available');
                }
    ...
    private loadAndEmitCache() {
        ...
        try {
            ...
            // 记录最后接受的点，用于去重对比
            // @ts-expect-error - FIXME: Type 'GeoPoint' is not assignable to type '{ lat: number; lng: number; - [Ticket-202603-SchemaSync] baseline exemption
    ...
    private handleNativeError(err: AMapLocationError) {
        ...
        // ✅ Bridge 层也拦截鉴权错误，防止其传入 onError 回调后引发 Promise Rejection
        const isAuthError = 
            String(err.code) === '7' ||
        ...
        if (isAuthError) {
            logWarn({ phase: 'native-location-error', reason: `AMap auth error suppressed: code=${err.code} message=${err.message}` });
            // 降级到 web fallback，不向外部回调抛出 error
            this.callbacks.onStatusChange('locating'); // 维持 locating 状态，不进入 error
            return;
        }
    ...
    private handleWatchUpdate(point: GeoPoint, locationType?: number) {
        ...
        // --- 基于时间的推送节流 (UI/Binder 保护) ---
        // 关键重构：显式拆分"首次有效点"和"后续点"的逻辑
        const now = Date.now();
        const isFirstFreshFix = !this.hasFreshFix;
        ...
        // 非首次点：执行正常的 shouldThrottle 节流与静默更新逻辑
        const shouldThrottle = this.watchMode === 'browse' && 
          (now - this.lastEmittedTime < AMapLocationBridge.EMIT_THROTTLE_MS);
        
        if (shouldThrottle) {
            // 仅静默更新内部追踪，不触发 callbacks
            ...
            logInfo({ 
              phase: 'watch-update-throttled', 
              reason: `Throttled (${now - this.lastEmittedTime}ms < ${AMapLocationBridge.EMIT_THROTTLE_MS}ms)`, 
    ...
    async fastFixWithRecoveryWindow(opts?: {
        ...
        try {
            ...
            await this.switchWatchMode('browse', { interval: 1000, distanceFilter: 3 });
    ...
    async switchWatchMode(
        ...
        await this.startWatch({
            ...
            distanceFilter: options?.distanceFilter ?? (targetMode === 'running' ? 3 : 5),
        });
