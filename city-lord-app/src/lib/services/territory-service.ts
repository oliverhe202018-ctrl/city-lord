...
import { isLoopClosed, calculatePathDistance, LOOP_CLOSURE_THRESHOLD_M, MIN_LOOP_POINTS, GeoPoint,  } from '@/lib/geometry-utils';
...
export const TerritoryService = {
    ...
    async createFromPath(
        ...
        // 首尾吸附：将起点坐标副本追加到末尾，确保 GeoJSON LinearRing 合法
        const coords = points.map(p => [p.lng, p.lat]);
        const firstCoord = coords[0];
        const lastCoord = coords[coords.length - 1];
        if (firstCoord[0] !== lastCoord[0] || firstCoord[1] !== lastCoord[1]) {
            coords.push([...firstCoord]);
            console.log(`[TerritoryService] 首尾吸附闭合：gap=${Math.round(loopCheck.gapDistance)}m`);
        }
