...
export function RunSummaryView({
  ...
  // Reward Polling Logic — 基于 ref 的轮询超时机制
  // 最大重试次数 15 次（每 2s 一次，总计 30 秒），超时后强制停止轮询
  const pollCountRef = useRef(0);
  const { data: rewardStatus, isError } = useQuery({
    queryKey: ['run-rewards', runId],
    queryFn: async () => {
      if (!runId) return null;
      pollCountRef.current += 1;
      if (pollCountRef.current > 15) {
        console.warn('[RunSummary] 奖励轮询超时（15次/30s），强制停止');
        toast.warning('奖励处理超时，请稍后在成就页面查看');
        return null;
      }
          ...
          <div className="mx-4 mb-4">
            {photoUrl ? (
              <div className="relative w-full bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100">
                <img src={photoUrl} alt="跑步照片" width={500} height={200} className="w-full h-48 object-cover" unoptimized />
