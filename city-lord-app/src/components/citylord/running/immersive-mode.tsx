...
const RunControlButtons = memo(function RunControlButtons({
  ...
  return (
    <div className={pairWrapperClassName}>
      <button
        type="button"
        className={`touch-none select-none ${pairButtonClassName} ${isEndPressing ? "bg-red-700" : "bg-red-500"}`}
      ...
      <button
        type="button"
        className={`touch-none select-none ${singleLabelClassName || singleButtonClassName}`}
        onClick={onPauseToggle}
      >
        继续跑步
      </button>
...
function ImmersiveRunningModeInner({
  ...
  const [sessionRunId, setSessionRunId] = useState<string>(() => uuidv4())
  ...
  const loadTerritories = useCallback(async () => {
    ...
    try {
      const res = await apiFetch(`/api/territory/list?cityId=${currentCity.id}`, {
        credentials: 'include'
      })
      ...
      try {
        ...
        if (res.success && res.data?.isSettled) {
          ...
          if (terrRes.success && terrRes.data?.territoryId) {
            ...
            console.log(`[ID Swap] Success: ${territoryId}`);
          } else {
        ...
        if (elapsed >= MAX_POLL_DURATION_MS) {
          ...
          setIsSettlementLoading(false);
          clearInterval(intervalId);
          console.log(`[Settlement] Polling stopped (timeout after ${MAX_POLL_DURATION_MS}ms, ${attempts} attempts).`);
        }
      } catch (err) {
  ...
  const performCleanExit = useCallback(async () => {
    clearRecovery?.()
    try { localStorage.removeItem('CURRENT_RUN_RECOVERY') } catch {}
    finalize?.()
    const { useGameStore } = await import('@/store/useGameStore')
    useGameStore.getState().resetRunState()
  }, [clearRecovery, finalize])
