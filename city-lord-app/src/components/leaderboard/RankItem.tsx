...
export function RankItem({ data }: RankItemProps) {
  ...
  const scoreDisplay = useMemo(() => {
    if (typeof score === 'number') {
      return formatArea(score);
    }
    return { value: String(score), unit: '', fullText: String(score) };
  }, [score]);
  ...
  const handleClick = () => {
    if (data.id) {
      openUserProfile(navigate, data.id, window.location.pathname + window.location.search);
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="text-right ml-2">
        ...
        <div className="text-[10px] text-muted-foreground mt-0.5">{scoreDisplay.unit || '—'}</div>
      </div>
    </div>
  );
}
