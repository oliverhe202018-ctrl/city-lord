...
// 补全所有漏掉的图标
import { Users, Plus, Search, Lock, Unlock, TrendingUp, MapPin, LogOut, BarChart3, Scale, Swords, Target, Rocket, TrendingDown, CheckCircle2, UserPlus, MessageSquare, // 修复 MessageSquare is not defined
                  ...
                  <div className="grid grid-cols-4 w-full">
                    {[
                      ...
                      <button
                        ...
                        className={`flex flex-col items-center justify-center py-3 gap-1 relative transition-colors ${activeTab === tab.id ? 'text-primary' : 'text-muted-foreground hover:text-foreground'
                          }`}
                      >
                      ...
                      <div className="space-y-2">
                        {participants.map((p, index) => (
                          <button
                            ...
                            onClick={() => {
                              if (p?.id) {
                                navigate(`/profile/user?userId=${p.id}`);
                                onClose();
                              }
                            }}
                          ...
                          >
                            <div className="flex items-center gap-3 flex-1 min-w-0">
                              <div className={`flex items-center justify-center w-6 font-bold font-mono text-sm flex-shrink-0 ${index === 0 ? 'text-yellow-400' : index === 1 ? 'text-gray-300' : index === 2 ? 'text-amber-600' : 'text-muted-foreground'
                                }`}>
                                #{index + 1}
                              </div>
                                ...
                                {groupEvents.map((event) => (
                                  <div key={event.id} className="relative group">
                                    ...
                                    <div
                                      onClick={() => {
                                        onClose();
                                        navigate(`/run/detail?id=${event.id}`);
                                      }}
                                        ...
                                        <div className="w-16 h-16 rounded-lg bg-muted overflow-hidden border border-border flex-shrink-0 flex items-center justify-center">
                                          {event.thumbnail ? (
                                            <img
                                              ...
                                              width={64}
                                              className="w-full h-full object-cover"
                                              unoptimized={(event.thumbnail as string).startsWith('blob:')}
                                            />
                                          ) : (
