...
export function FactionSelector({ initialUser }: FactionSelectorProps) {
  ...
  const checkFactionStatus = useCallback(async (currentUser?: any) => {
    ...
    // ✅ 新增：sessionStorage 快速短路，避免因 INITIAL_SESSION 重复弹出
    if (typeof window !== 'undefined' && sessionStorage.getItem('faction_selected')) return
    ...
    if (user) {
      ...
      if (profile?.faction) {
        // ✅ 新增：已有阵营时写入 sessionStorage 防止下次重复检查
        sessionStorage.setItem('faction_selected', '1')
        return
      }
      setIsOpen(true)
      loadStats()
  ...
  const handleJoin = async (faction: Faction) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        sessionStorage.setItem('faction_selected', '1')  // ← 新增这一行
        ...
        // 3. 刷新状态中心以更新数据，避免无谓的 window.location.reload() 全局刷新
