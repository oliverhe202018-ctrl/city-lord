...
export function GaodeMap3D({
  ...
  // ✅ 优化：使用 useCallback 包装 addLog 函数，避免每次渲染重新创建
  const addLog = useCallback((msg: string) => {
    console.log(`[GaodeMap3D] ${msg}`)
    // 仅在开发环境下更新 debugLog，生产环境不触发重渲染
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // ✅ 修复：loca.animate.start() 只调用一次，而不是每帧调用
        loca.animate.start()
...
// @ts-expect-error - Baseline exemption for pre-existing schema mismatch - [Ticket-202603-SchemaSync] baseline exemption
    ...
    if (closedPolygons && closedPolygons.length > 0) {
        // ... (Keep polygon logic similar to map territory overlay if needed, but GaodeMap3D usually for tracking)
    }
