...
export function RunHistoryList({ userId, initialRuns, initialCursor }: RunHistoryListProps) {
    ...
    const loadRuns = useCallback(async (existingCursor?: string) => {
        try {
            ...
            deduplicated.sort((a, b) => new Date(b.createdAt || b.created_at).getTime() - new Date(a.createdAt || a.created_at).getTime());
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div className="text-xs text-muted-foreground">
                            {new Date(run.createdAt || run.created_at).toLocaleDateString('zh-CN')} ·{' '}
                            {new Date(run.createdAt || run.created_at).toLocaleTimeString('zh-CN', {
                                hour: '2-digit',
                                minute: '2-digit',
                            })}
                        </div>
