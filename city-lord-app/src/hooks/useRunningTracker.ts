...
// ============================================================================
// ⚠️ TODO: useRunningTracker 巨型 Hook 拆分规划 (当前 ~1500 �?
//
// 当前 Hook 承担了过多职责，建议在下一迭代拆分为三层独�?Hook�?
//
// 1. useRunClock (纯计�?+ 暂停/恢复)
...
//
// 拆分�?useRunningTracker 仅作为协调层 (orchestrator)，将上述三个 Hook
// 组合为统一�?RunningStats 接口，预计可降至 300 行以内�?
// ============================================================================
...
export function useRunningTracker(isRunning: boolean, userId?: string): RunningStats {
  ...
  // 双层存储架构：Preferences 仅保留结算元数据 + 最�?100 个点作为安全�?
  // 全量原始轨迹由原�?Room DB 保管，前端恢复时通过 hydrateOfflinePoints 拉取
  ...
  // 渲染抽稀：使�?Douglas-Peucker 算法异步计算简化路径，
  // TrajectoryLayer 仅绑�?displayPath，避免全量原始点 GPU 渲染压力�?
  // 超过 30 点后，每新增 5 个点触发一�?simplifyPath，其余时间同步追�?
  ...
  useEffect(() => {
    ...
    const scheduleSimplify = () => {
      ...
      // 点数 <= 30，直接同步执�?setDisplayPath 保证首段秒级无延迟渲�?
  ...
  const handleLocationUpdate = useCallback((
    ...
    // ================================================================
    // 🛡�?TIMESTAMP SLIDING WINDOW FILTER (全局 5 秒滑动窗口去重与回拨拦截)
    // ================================================================
    ...
    if (isOverlap || isRegressive) {
      console.debug(
        `[Timestamp Sliding Window Filter] �?useRunningTracker 拦截重叠/回拨�? ts=${now}, maxTs=${maxTs}, isOverlap=${isOverlap}, isRegressive=${isRegressive}`
      );
      return;
    }
    ...
    // ================================================================
    // 🛡�?TIMESTAMP DEDUP (防后台缓存点重复注入)
    // 后台定位插件可能在唤醒时补发历史缓存点，若时间戳不晚于已处理点则丢弃
    // 增加 CLOCK_DRIFT_TOLERANCE_MS 容忍窗口，防�?NTP 同步/跨时区导致的时钟回拨误杀
    // ================================================================
    ...
    // ================================================================
    // 🛡�?THREE-LAYER GPS ANTI-JITTER INTERCEPTOR
    // Applied BEFORE smoothing, path appending, or distance calculation
    //
    // 展示流与记录流分离：
    // - Anchor 阶段（pathRef 为空）：精度 <= 50m 的点可作为首个锚�?
    // - 追踪阶段：精�?<= 30m 的点才参与距离结�?
    // - Warm-up 期间：所有通过精度检查的点仅更新 UI（currentLocation），
    //   绝对不写�?pathRef，直到连续收到高精度点解�?warm-up
    // ================================================================
    ...
    if (isAwaitingAnchor) {
      ...
      // 起跑初期（前 20 个点）允许放行精度在展示门槛�?00m）内的点，以便立即渲染轨迹，并在后台平滑修复
    ...
    if (pathRef.current.length >= 2) {
      ...
      // 1. 提升距离门槛�?3m（与位移防抖对齐），防止长线段限制误杀小幅毛刺
      if (dAB > 3.0 && dBC > 3.0 && dAC > 0) {
        ...
        // 仅当横向偏离�?> 3.0m 且方向角 > 135°，且伴随速度异常（如单段 > 7.0 m/s）时，才判定为极端回头漂移尖角并过滤
    ...
    // 展示流与记录流分离核心逻辑�?
    // warm-up 期间，所有通过精度检查的点仅更新 currentLocation �?UI 渲染�?
    // 绝对不写�?pathRef 参与距离结算�?
    if (isWarmingUpRef.current) {
      ...
      if (now - firstPointAtRef.current > WARMUP_TIMEOUT_MS || validPointsCountRef.current >= MIN_WARMUP_POINTS) {
        ...
        // [优化] 注释/删除此行，必须保�?finalLoc 作为后续轨迹过滤参考点，防止首点丢失与位移防抖误杀
    ...
    if (prevLoc && prevLoc.timestamp) {
      ...
      if (speedResult.flag) {
        ...
        if (consecutiveSpeedFlags >= 3 && consecutiveSpeedFlags % 3 === 0) {
          toast.warning('⚠️ 检测到异常移动，本次跑步可能被标记为疑似作�?);
        }
      }
    }
    ...
    // 后台线性插值算�?(Linear Interpolation Back-Smoothing)
    if (accuracy != null && accuracy <= GPS_TRACKING_ACCURACY_METERS) {
      ...
      if (currentPath.length >= 3) {
        ...
        if (firstHighAccIndex > 1) {
          console.log(`[GPS-Repair] 🛠�?Found first high-accuracy point at index ${firstHighAccIndex}. Smoothing preceding points...`);
          ...
          if (totalDuration > 0) {
            ...
            for (let i = 1; i < firstHighAccIndex; i++) {
              ...
              (pt as any).accuracy = GPS_TRACKING_ACCURACY_METERS; // 提升精度，解除脏点状�?
            }
 
            // 里程校准：重新计算整�?Path 的累计哈弗辛距离，覆盖更�?distanceRef.current �?setDistance 状态，严防里程虚高
            let newTotalDist = 0;
            for (let i = 1; i < currentPath.length; i++) {
    ...
    // ========================================================================
    // 每累�?5 �?GPS 点触发一�?extractValidLoops 自交检测，
    // 覆盖 P 形环�? 字形等复杂轨迹，确保 UI 面积实时跳动�?
    // 滑动窗口优化：路径超�?1000 点时仅扫描最�?1000 点，阻断 O(n²) 增长�?
    // 1000 个点 × 3m 间隔 �?3000m，正常跑步不会跑�?>3000m 不自交的环�?
    // ========================================================================
    ...
    if (realtimeLoopCheckCounterRef.current % 15 === 0 && pathRef.current.length >= 6) {
      ...
      for (const loop of detectedLoops) {
        ...
        // key = 质心精确�?4 位小�?(�?11 米精�? + 面积取整，同一个环重复检测时 key 不变�?字两环由于质�?面积不同�?key 不同
  ...
  const hydrateOfflinePoints = useCallback(async (sessionId: string) => {
    ...
    try {
      ...
      // 精确 sinceTimestamp：优先使�?pathRef 最后一个点的时间戳
      ...
      if (offlinePoints.length === 0) {
        console.log('[Hydrate] 无离线记录需要追�?);
        clearTimeout(hydrateTimeout);
        isHydratingRef.current = false;
        return;
      }

      console.log(`[Hydrate] 拉取�?${offlinePoints.length} 条离线定位记�?(capped=${res.capped}), sessionId=${sessionId}`);
        ...
        if (chunkIndex >= totalChunks) {
          ...
          if (missedMilestones.length > 0) {
            ...
            for (const k of missedMilestones) {
              ...
              const msg = `领主，您已奔�?${k} 公里！当前配�?${segmentPaceStr}，势如破竹，请继续保持！`;
          ...
          if (pendingLivePointsRef.current.length > 0) {
            ...
            console.log(`[Hydrate] 追帧完成，开始注入缓冲队列中�?${pendingPoints.length} 个实时点`);
      ...
      clearTimeout(hydrateTimeout);
      isHydratingRef.current = false;
      console.warn('[Hydrate] �?Room 黑匣子追帧失败（可能�?Web 环境�?', e);
    }
  ...
  const setAnchorPoint = useCallback((anchor: GeoPoint) => {
    ...
    // 同步�?anchor 坐标作为首点推入 pathRef.current 并更�?state，解决起�?3m 轨迹断层问题
  ...
  // ========================================================================
  // 入口 2：启动强制补�?�?解决"起步即锁�?短路问题
  // isRunning 变为 true 后，等待 Native Service 就绪，立刻追帧一�?
  // ========================================================================
  useEffect(() => {
    if (!isRunning || isStoppingRef.current) return;
  ...
  // ========================================================================
  // 入口 3：兜底轮�?�?�?30 秒检查一�?Room 是否有堆积点�?
  // 终极保障：即�?appStateChange 未触发，也能定期补帧
  // ========================================================================
  useEffect(() => {
  ...
  useEffect(() => {
    if (isRunning) {
      ...
      const performRecovery = async () => {
        ...
        // 注入 currentRunId �?useLocationStore，供亮屏追帧（Hydration）使�?
  ...
  const recoverUnfinishedSession = useCallback(async () => {
    try {
      ...
      // 4小时�?4400000ms）内未结算的 Session 自愈
      if (elapsedMs < 4 * 60 * 60 * 1000) {
        ...
        toast.success("已自动恢�?4 小时内未结算的跑步记录！");
        return data;
      } else {
