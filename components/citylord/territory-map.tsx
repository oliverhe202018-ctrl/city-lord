...
import { formatAreaFromTerritoryCount } from "@/lib/citylord/area-utils"
...
function generateTerritoryGrid(): TerritoryCell[] {
...
export function TerritoryMap() {
  const [territories] = useState<TerritoryCell[]>(generateTerritoryGrid)
  ...
  const territorySize = 28
  const territoryWidth = territorySize * 2
  const territoryHeight = Math.sqrt(3) * territorySize
    ...
    <div className="relative h-full w-full overflow-hidden bg-[#1a1a1a]">
      ...
      <svg className="absolute inset-0 h-full w-full" viewBox="0 0 500 800" preserveAspectRatio="xMidYMid slice">
        ...
        {territories.map((territory) => {
          const xPos = territory.x * territoryWidth * 0.75 + 50
          const yPos = territory.y * territoryHeight + (territory.x % 2 === 1 ? territoryHeight / 2 : 0) + 50
          
          const points = Array.from({ length: 6 }, (_, i) => {
            const angle = (Math.PI / 3) * i + Math.PI / 6
            return `${xPos + territorySize * Math.cos(angle)},${yPos + territorySize * Math.sin(angle)}`
          }).join(" ")
          ...
          return (
            <polygon
              key={territory.id}
              points={points}
              className={`${getTerritoryColor(territory.owner)} transition-all duration-300`}
              strokeWidth="1.5"
              filter={territory.owner !== "neutral" ? `url(#glow-${territory.owner === "user" ? "green" : "red"})` : undefined}
            />
          )
        })}
      </svg>
      ...
      {(() => {
        const myArea = formatAreaFromTerritoryCount(24)
        const contestedArea = formatAreaFromTerritoryCount(8)
        return (
          <>
