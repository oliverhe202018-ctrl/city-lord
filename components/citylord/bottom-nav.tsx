        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              onClick={() => {
                closeDrawer()
                // 强制关闭领地详情抽屉（setIsDetailSheetOpen 存于 MapInteractionContext，closeDrawer 无法触达）
                // BottomNav 渲染在 MapRoot/MapInteractionProvider 树之外，无法直接调用 useMapInteraction()
                // 改用全局 CustomEvent 让树内的 TerritoryLayer/AMapView 监听并关闭
