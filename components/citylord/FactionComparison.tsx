...
export function FactionComparison({ initialData, redArea: propRedArea, blueArea: propBlueArea, dailyStat }: FactionComparisonProps) {
  ...
  // Dynamic buff percentage from backend API (with local fallback)
  const [buffPercentage, setBuffPercentage] = useState(0)
  useEffect(() => {
    const fetchBalanceConfig = async () => {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_SERVER || ''}/api/faction/balance-config`, { credentials: 'include' })
        if (res.ok) {
          const config = await res.json()
          setBuffPercentage(config.buffPercentage ?? 0)
        } else {
          throw new Error('API failed')
        }
      } catch {
        // Fallback: calculate locally using the same utility
        const { multiplier } = calculateFactionBalance(
          propRedArea ?? initialData?.red_area ?? 0,
          propBlueArea ?? initialData?.blue_area ?? 0
        )
        setBuffPercentage(Math.round((multiplier - 1) * 100))
      }
    }
    fetchBalanceConfig()
  }, [propRedArea, propBlueArea, initialData])
