...
interface AdminRoomsPageClientProps {
  initialRooms: Room[]
  initialError?: string | null
}

export default function AdminRoomsPageClient({ initialRooms, initialError = null }: AdminRoomsPageClientProps) {
  ...
  const toggleBan = async (room: Room) => {
    ...
    try {
      ...
      setRooms((prev) => prev.map((r) => (r.id === room.id ? { ...r, is_banned: newBanStatus } : r)))
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>显示所有创建的房间。</CardDescription>
        </CardHeader>
        <CardContent>
          {rooms.length === 0 ? (
            ...
            <Table>
              ...
              <TableBody>
                {rooms.map((room) => (
                  <TableRow key={room.id} className={room.is_banned ? 'bg-muted/50 opacity-60' : ''}>
                    ...
                    <TableCell>
                      <div className="flex flex-col">
                        ...
                        <span className="font-mono text-[10px] text-muted-foreground">{room.host_id.slice(0, 8)}...</span>
                      </div>
                    </TableCell>
                    <TableCell>
