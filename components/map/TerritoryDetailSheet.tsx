...
export interface TerritoryCellData {
  territoryId: string
...
interface TerritoryDetailSheetProps {
  isOpen: boolean
  onClose: () => void
  cell: TerritoryCellData | null
  onNavigate: (cell: TerritoryCellData) => void
  onAction: (cell: TerritoryCellData) => void
}

export function TerritoryDetailSheet({ isOpen, onClose, cell, onNavigate, onAction }: TerritoryDetailSheetProps) {
  if (!cell) return null
          ...
          <div className="p-4 flex-1 flex flex-col">
            <div className="flex items-start justify-between mb-6">
              <div>
                ...
                <p className="text-sm text-white/50 font-mono">{cell.territoryId}</p>
              </div>
              {cell.level && (
