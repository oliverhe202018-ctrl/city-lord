...
/**
 * GET /api/v1/club/dynamics?clubId=xxx
    ...
    try {
        ...
        if (!user) {
            return NextResponse.json(
                { success: false, error: 'NOT_AUTHENTICATED', message: '请先登录' },
                { status: 401 }
            )
        }
        ...
        if (!clubId) {
            return NextResponse.json(
                { success: false, error: 'INVALID_PARAMS', message: '缺少 clubId 参数' },
                { status: 400 }
            )
        }
        ...
        const activities = await prisma.club_activities.findMany({
            ...
            take: 10,
            include: {
                // @ts-expect-error - FIXME: Object literal may only specify known properties
                profiles: {
                    select: { nickname: true, avatar_url: true },
                },
            },
        ...
        for (const r of runs) {
            ...
            const area_display = rawArea < 1000
                ? `${Math.round(rawArea)} ㎡`
                : rawArea < 1000000
            ...
            items.push({
                ...
                title: '领地拓展',
                description: `${r.profiles?.nickname || '成员'} 跑步开拓了 ${area_display} 领地`,
        ...
        for (const b of badges) {
            items.push({
                ...
                title: '成就达成',
                // @ts-expect-error - FIXME: Property 'badge_name' does not exist
        ...
        console.error('[GET /api/v1/club/dynamics] Error:', error)
        return NextResponse.json(
            { success: false, error: 'INTERNAL_ERROR', message: '获取俱乐部动态失败' },
            { status: 500 }
        )
    }
}
