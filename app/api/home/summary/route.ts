...
async function fetchLeaderboard(userId: string, scope: string): Promise<{
    ...
    const userProfile = await prisma.profiles.findUnique({
        where: { id: userId },
        select: { total_area: true, nickname: true, city_code: true },
    });

    const whereCondition: Record<string, unknown> = { is_active: true };
    if (scope === 'city' && userProfile?.city_code) {
        whereCondition.city_code = userProfile.city_code;
    }
