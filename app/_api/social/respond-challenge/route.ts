import { NextResponse } from 'next/server'
import { createClient } from '@/lib/supabase/server'
import { respondToChallenge } from '@/app/actions/social'

export async function POST(request: Request) {
  try {
    const supabase = await createClient()
    const { data: { user } } = await supabase.auth.getUser()

    if (!user) {
      return NextResponse.json({ error: 'Not authenticated' }, { status: 401 })
    }

    const body = await request.json()
    const { challengeId, accept } = body || {}

    if (!challengeId || typeof accept !== 'boolean') {
      return NextResponse.json({ error: 'challengeId and accept required' }, { status: 400 })
    }

    const result = await respondToChallenge(challengeId, accept)
    return NextResponse.json(result)
  } catch (error: any) {
    console.error('respondToChallenge error:', error)
    return NextResponse.json({ success: false, error: error.message || 'Failed to respond' }, { status: 500 })
  }
}
