...
const ROOMS_LIST_CACHE_KEY = 'rooms:list:open'
const ROOMS_LIST_CACHE_TTL = 15
...
export async function createRoom(data: CreateRoomData) {
  try {
    ...
    await invalidateCache(ROOMS_LIST_CACHE_KEY)
...
export async function joinRoomByCode(code: string) {
  try {
    ...
    const { data: room, error: findError } = await (supabase
      .from('rooms' as any) as any)
      .select('id, max_participants, participants_count')
      .eq('invite_code', code)
      .single()
    ...
    // 3. Check capacity
    const currentCount = (room as any).participants_count || 0
...
export async function joinRoom(roomId: string, password?: string) {
  ...
  try {
    ...
    const { data: room, error: fetchError } = await (supabase
      .from('rooms' as any) as any)
      .select('max_participants, is_private, password, participants_count')
      .eq('id', roomId)
      .single()
