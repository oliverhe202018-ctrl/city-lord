...
export async function createClub(data: {
  ...
  try {
    ...
    const { data: existingClub } = await getSupabaseAdmin()
      .from('clubs')
      .select('id')
      .eq('name', data.name)
      .maybeSingle()
...
export async function getPendingClubs(): Promise<{ success: true; data: PendingClubDTO[] } | { success: false; error: string }> {
  try {
    ...
    const { data: pendingClubs, error: pendingError } = await getSupabaseAdmin()
      .from('clubs')
...
export async function getApprovedClubs(): Promise<{ success: true; data: ApprovedClubDTO[] } | { success: false; error: string }> {
  try {
    ...
    const { data: approvedClubs, error } = await getSupabaseAdmin()
      .from('clubs')
      .select('*, profiles_clubs_owner_idToprofiles:profiles!clubs_owner_id_fkey(nickname, avatar_url)')
      .eq('status', 'active')
      .order('created_at', { ascending: false })
...
export async function approveClub(clubId: string) {
  ...
  const { data: club, error } = await getSupabaseAdmin()
    .from('clubs')
...
export async function leaveClub(clubId: string) {
  ...
  if (FF_CLUB_EXIT_DETACHMENT_ENABLED) {
    try {
      const { error: detachError } = await getSupabaseAdmin().rpc('detach_club_territories', {
        p_user_id: user.id,
        p_club_id: clubId
      });
      if (detachError) {
...
export async function getClubRankStats(clubId: string) {
  try {
    ...
    const { count: globalRankCount } = await getSupabaseAdmin()
      .from('clubs')
      .select('*', { count: 'exact', head: true })
    ...
    if (province) {
      const { count: provincialRankCount } = await getSupabaseAdmin()
        .from('clubs')
        .select('*', { count: 'exact', head: true })
        .gt('total_area', totalArea)
        .eq('status', 'active')
...
export async function getClubDetailsById(id: string) {
  return cachedFetch(`club_detail:${id}`, 120, async () => {
    ...
    const { data: membersResult } = await getSupabaseAdmin()
      .from('club_members')
    ...
    const { count: memberCountResult } = await getSupabaseAdmin()
      .from('club_members')
      .select('*', { count: 'exact', head: true })
      .eq('club_id', id)
      .eq('status', 'active')
