...
export async function sendSmsCode(
    ...
    try {
        ...
        if (type === 'register') {
            ...
            const res = await getSupabaseAdmin().auth.admin.generateLink({
                type: 'signup',
                email: virtualEmail,
                password,
                options: {
        ...
        if (targetUser) {
            await getSupabaseAdmin().auth.admin.updateUserById(targetUser.id, {
                app_metadata: {
...
export async function verifySmsCode(
    ...
    try {
        ...
        if (type === 'login') {
            const linkRes = await getSupabaseAdmin().auth.admin.generateLink({
                type: 'magiclink',
                email: virtualEmail,
                options: {
