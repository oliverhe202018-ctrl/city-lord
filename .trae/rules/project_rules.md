...
- **终端命令格式**: 所有终端命令默认使用 **PowerShell** 语法格式（非 Bash/CMD）。
  > 规则：执行命令时使用 PowerShell 语法，如分号 `;` 连接命令、`Get-ChildItem` 替代 `ls -la`、`Invoke-RestMethod` 替代 `curl` 等。
- **APK 打包后自动上传**: 每次完成 APK 打包后，必须自动上传至 tmpfile.link 免费云端并返回下载链接。
  > 规则：使用 PowerShell 脚本上传 `android/app/build/outputs/apk/debug/app-debug.apk` 到 `https://tmpfile.link/api/upload`，解析返回的 `downloadLink` 并展示给用户。文件 7 天后自动删除。
